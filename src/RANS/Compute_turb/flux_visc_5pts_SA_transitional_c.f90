!==============================================================================
subroutine flux_visc_5pts_SA_transition_c
!==============================================================================
     ...
     do j=ndy_v_r,nfy_v_r
        do i=ndx_vi_r,nfx_vi_r
            Knutil(i,j,k) = (a5(1)*(Fnutil(i+1,j,k)-Fnutil(i-1,j,k) ) &
                              +a5(2)*(Fnutil(i+2,j,k)-Fnutil(i-2,j,k)))*ijacob_v(i,j) + Knutil(i,j,k)
            Kgamma(i,j,k) = (a5(1)*(Fgamma(i+1,j,k)-Fgamma(i-1,j,k) ) &
                              +a5(2)*(Fgamma(i+2,j,k)-Fgamma(i-2,j,k)))*ijacob_v(i,j) + Kgamma(i,j,k)
            Kre_theta(i,j,k) = (a5(1)*(Fre_theta(i+1,j,k)-Fre_theta(i-1,j,k) ) &
                              +a5(2)*(Fre_theta(i+2,j,k)-Fre_theta(i-2,j,k)))*ijacob_v(i,j) + Kre_theta(i,j,k)
        enddo
     enddo
  enddo
     ...
     do j=ndy_vi_r,nfy_vi_r
        do i=ndx_v_r,nfx_v_r
            Knutil(i,j,k) = (a5(1)*(Gnutil(i,j+1,k)-Gnutil(i,j-1,k)) &
                              +a5(2)*(Gnutil(i,j+2,k)-Gnutil(i,j-2,k)) )*ijacob_v(i,j) + Knutil(i,j,k)
            Kgamma(i,j,k) = (a5(1)*(Ggamma(i,j+1,k)-Ggamma(i,j-1,k)) &
                              +a5(2)*(Ggamma(i,j+2,k)-Ggamma(i,j-2,k)) )*ijacob_v(i,j) + Kgamma(i,j,k)
            Kre_theta(i,j,k) = (a5(1)*(Gre_theta(i,j+1,k)-Gre_theta(i,j-1,k)) &
                              +a5(2)*(Gre_theta(i,j+2,k)-Gre_theta(i,j-2,k)) )*ijacob_v(i,j) + Kre_theta(i,j,k)
        enddo
     enddo
  enddo
     ...
     do j=ndy_v_r,nfy_v_r
        do i=ndx_v_r,nfx_v_r
            Knutil(i,j,k) = (a5(1)*(Hnutil(i,j ,k+1)-Hnutil(i,j,k-1)) &
                              +a5(2)*( Hnutil(i,j,k+2)-Hnutil(i,j,k-2)))*idz_v(k) + Knutil(i,j,k)
            Kgamma(i,j,k) = (a5(1)*(Hgamma(i,j ,k+1)-Hgamma(i,j,k-1)) &
                              +a5(2)*( Hgamma(i,j,k+2)-Hgamma(i,j,k-2)))*idz_v(k) + Kgamma(i,j,k)
            Kre_theta(i,j,k) = (a5(1)*(Hre_theta(i,j ,k+1)-Hre_theta(i,j,k-1)) &
                              +a5(2)*( Hre_theta(i,j,k+2)-Hre_theta(i,j,k-2)))*idz_v(k) + Kre_theta(i,j,k)
        enddo
     enddo
  enddo
