...
!===============================================================
subroutine update_var_SA_transition
    !===============================================================
    ! Update conservative nutilde at next RK step
    !===============================================================
      use mod_flow
    ...
    !               uvar(i,j,k,2) = nutil_n(i,j,k)
                  nutil_n(i,j,k) = nutil(i,j,k)-dt_local(i,j,k)*crk(irk)*(Knutil(i,j,k)-Sterm(i,j,k))
                  rhogamma_n(i,j,k) = rho_gamma(i,j,k)-dt_local(i,j,k)*crk(irk)*(Kgamma(i,j,k)-Sgamma(i,j,k))
                  ! Limit rhogamma_n to rho_n or 0:
                  if (rhogamma_n(i,j,k) .lt. 0.0_wp)then
                    rhogamma_n(i,j,k) = 0.0_wp 
                  elseif(rhogamma_n(i,j,k) .gt. rho_n(i,j,k))then
                    rhogamma_n(i,j,k) = rho_n(i,j,k)
                  endif
                  rhore_theta_n(i,j,k) = rhore_theta(i,j,k)-dt_local(i,j,k)*crk(irk)*(Kre_theta(i,j,k)-Stheta(i,j,k))
                  if (nutil_n(i,j,k).lt.0.0_wp) nutil_n(i,j,k)=low_value
               enddo
            enddo
         enddo
    ...
    !               uvar(i,j,k,2) = nutil_n(i,j,k)
                  nutil_n(i,j,k) = nutil(i,j,k)-alpha*(Knutil(i,j,k)-Sterm(i,j,k))
                  rhogamma_n(i,j,k) = rho_gamma(i,j,k)-alpha*(Kgamma(i,j,k)-Sgamma(i,j,k))
                  rhore_theta_n(i,j,k) = rhore_theta(i,j,k)-alpha*(Kre_theta(i,j,k)-Stheta(i,j,k))
                  if (nutil_n(i,j,k).lt.0.0_wp) nutil_n(i,j,k)=low_value
               enddo
            enddo
         enddo
    ...
    end subroutine update_var_SA_transition
    
    !========================================================================
    subroutine update_varn_artvisc_SA_transition
    !========================================================================
    ! Update conservative nutilde after application of numerical dissipation
    !========================================================================
      use mod_flow
    ...
    !               uvar(i,j,k,2) = nutil_n(i,j,k)
                  nutil_n(i,j,k) = nutil(i,j,k)-dt_local(i,j,k)*Knutil(i,j,k)
                  rhogamma_n(i,j,k) = rho_gamma(i,j,k)-dt_local(i,j,k)*Kgamma(i,j,k)
                  rhore_theta_n(i,j,k) = rhore_theta(i,j,k)-dt_local(i,j,k)*Kre_theta(i,j,k)
                  if (nutil_n(i,j,k).lt.0.0_wp) nutil_n(i,j,k)=low_value
               enddo
            enddo
         enddo
    ...
    !               uvar(i,j,k,2) = nutil_n(i,j,k)
                  nutil_n(i,j,k) = nutil(i,j,k)-deltat*Knutil(i,j,k)
                  rhogamma_n(i,j,k) = rho_gamma(i,j,k)-deltat*Kgamma(i,j,k)
                  rhore_theta_n(i,j,k) = rhore_theta(i,j,k)-deltat*Kre_theta(i,j,k)
    ...
    end subroutine update_varn_artvisc_SA_transition
    
    !===============================================================
    ! Update conservative variables to have dwi before application
    !  of IRS operator
    !===============================================================
    subroutine update_var_in_dw_SA_transition
      use mod_flow
      use mod_time
      use mod_forcing_bulk
      implicit none
    ...
    end subroutine update_var_in_dw_SA_transition
    
    !===============================================================================
    subroutine update_var_of_dw_SA_transition
    !===============================================================================
      !> Update conservative turbulent variables after application of IRS operator
    !===============================================================================
      use mod_flow         ! for: conservative variables
    ...
    end subroutine update_var_of_dw_SA_transition    