!===============================================================================
submodule (mod_TamDong3d_c3) smod_TamDong3d_edgey_c3
!===============================================================================
  !> author: XG
  !> date: April 2023
  !> Non-reflecting boundary conditions of Tam and Dong
  !> - 3D curvilinear version - routines for edges along y
!=============================================================================== 
...
contains
  ...
  !===============================================================================
    ...
    real(wp), dimension(1:ngh,ny,1:ngh) :: duksi,dvksi,dwksi,dpksi,drksi
    real(wp), dimension(1:ngh,ny,1:ngh) :: duphi,dvphi,dwphi,dpphi,drphi
    ...
    do k=1,ngh
       do i=1,ngh
          do j=ndy,nfy
             ...
             pt(i,j,k) = vg(i,j,k)*( &
                       ((dpksi(i,j,k)*ksi_x(i,j,k)+dpeta*eta_x(i,j,k)+dpphi(i,j,k)*phi_x(i,j,k))*sintcosp(i,j,k) &
                       +(dpksi(i,j,k)*ksi_y(i,j,k)+dpeta*eta_y(i,j,k)+dpphi(i,j,k)*phi_y(i,j,k))*sintsinp(i,j,k) &
                       +(dpksi(i,j,k)*ksi_z(i,j,k)+dpeta*eta_z(i,j,k)+dpphi(i,j,k)*phi_z(i,j,k))*costeta(i,j,k)  &
                       )*ijacob3(i,j,k) + pf(i,j,k)*ir(i,j,k) )
             ut(i,j,k) = vg(i,j,k)*( &
                       ((duksi(i,j,k)*ksi_x(i,j,k)+dueta*eta_x(i,j,k)+duphi(i,j,k)*phi_x(i,j,k))*sintcosp(i,j,k) &
                       +(duksi(i,j,k)*ksi_y(i,j,k)+dueta*eta_y(i,j,k)+duphi(i,j,k)*phi_y(i,j,k))*sintsinp(i,j,k) &
                       +(duksi(i,j,k)*ksi_z(i,j,k)+dueta*eta_z(i,j,k)+duphi(i,j,k)*phi_z(i,j,k))*costeta(i,j,k)  &
                       )*ijacob3(i,j,k) + uf(i,j,k)*ir(i,j,k) )
             vt(i,j,k) = vg(i,j,k)*( &
                       ((dvksi(i,j,k)*ksi_x(i,j,k)+dveta*eta_x(i,j,k)+dvphi(i,j,k)*phi_x(i,j,k))*sintcosp(i,j,k) &
                       +(dvksi(i,j,k)*ksi_y(i,j,k)+dveta*eta_y(i,j,k)+dvphi(i,j,k)*phi_y(i,j,k))*sintsinp(i,j,k) &
                       +(dvksi(i,j,k)*ksi_z(i,j,k)+dveta*eta_z(i,j,k)+dvphi(i,j,k)*phi_z(i,j,k))*costeta(i,j,k)  &
                       )*ijacob3(i,j,k) + vf(i,j,k)*ir(i,j,k) )
             wt(i,j,k) = vg(i,j,k)*( &
                       ((dwksi(i,j,k)*ksi_x(i,j,k)+dweta*eta_x(i,j,k)+dwphi(i,j,k)*phi_x(i,j,k))*sintcosp(i,j,k) &
                       +(dwksi(i,j,k)*ksi_y(i,j,k)+dweta*eta_y(i,j,k)+dwphi(i,j,k)*phi_y(i,j,k))*sintsinp(i,j,k) &
                       +(dwksi(i,j,k)*ksi_z(i,j,k)+dweta*eta_z(i,j,k)+dwphi(i,j,k)*phi_z(i,j,k))*costeta(i,j,k)  &
                       )*ijacob3(i,j,k) + wf(i,j,k)*ir(i,j,k) )
             rt(i,j,k) = vg(i,j,k)*( &
                       ((drksi(i,j,k)*ksi_x(i,j,k)+dreta*eta_x(i,j,k)+drphi(i,j,k)*phi_x(i,j,k))*sintcosp(i,j,k) &
                       +(drksi(i,j,k)*ksi_y(i,j,k)+dreta*eta_y(i,j,k)+drphi(i,j,k)*phi_y(i,j,k))*sintsinp(i,j,k) &
                       +(drksi(i,j,k)*ksi_z(i,j,k)+dreta*eta_z(i,j,k)+drphi(i,j,k)*phi_z(i,j,k))*costeta(i,j,k)  &
    ...
    do k=1,ngh
       do i=nxmnghp1,nx
          ...
          do j=ndy,nfy
             ...
             pt(l,j,k) = vg(l,j,k)*( &
                       ((dpksi(l,j,k)*ksi_x(i,j,k)+dpeta*eta_x(i,j,k)+dpphi(l,j,k)*phi_x(i,j,k))*sintcosp(l,j,k) &
                       +(dpksi(l,j,k)*ksi_y(i,j,k)+dpeta*eta_y(i,j,k)+dpphi(l,j,k)*phi_y(i,j,k))*sintsinp(l,j,k) &
                       +(dpksi(l,j,k)*ksi_z(i,j,k)+dpeta*eta_z(i,j,k)+dpphi(l,j,k)*phi_z(i,j,k))*costeta(l,j,k)  &
                       )*ijacob3(i,j,k) + pf(l,j,k)*ir(l,j,k) )
             ut(l,j,k) = vg(l,j,k)*( &
                       ((duksi(l,j,k)*ksi_x(i,j,k)+dueta*eta_x(i,j,k)+duphi(l,j,k)*phi_x(i,j,k))*sintcosp(l,j,k) &
                       +(duksi(l,j,k)*ksi_y(i,j,k)+dueta*eta_y(i,j,k)+duphi(l,j,k)*phi_y(i,j,k))*sintsinp(l,j,k) &
                       +(duksi(l,j,k)*ksi_z(i,j,k)+dueta*eta_z(i,j,k)+duphi(l,j,k)*phi_z(i,j,k))*costeta(l,j,k)  &
                       )*ijacob3(i,j,k) + uf(l,j,k)*ir(l,j,k) )
             vt(l,j,k) = vg(l,j,k)*( &
                       ((dvksi(l,j,k)*ksi_x(i,j,k)+dveta*eta_x(i,j,k)+dvphi(l,j,k)*phi_x(i,j,k))*sintcosp(l,j,k) &
                       +(dvksi(l,j,k)*ksi_y(i,j,k)+dveta*eta_y(i,j,k)+dvphi(l,j,k)*phi_y(i,j,k))*sintsinp(l,j,k) &
                       +(dvksi(l,j,k)*ksi_z(i,j,k)+dveta*eta_z(i,j,k)+dvphi(l,j,k)*phi_z(i,j,k))*costeta(l,j,k)  &
                       )*ijacob3(i,j,k) + vf(l,j,k)*ir(l,j,k) )
             wt(l,j,k) = vg(l,j,k)*( &
                       ((dwksi(l,j,k)*ksi_x(i,j,k)+dweta*eta_x(i,j,k)+dwphi(l,j,k)*phi_x(i,j,k))*sintcosp(l,j,k) &
                       +(dwksi(l,j,k)*ksi_y(i,j,k)+dweta*eta_y(i,j,k)+dwphi(l,j,k)*phi_y(i,j,k))*sintsinp(l,j,k) &
                       +(dwksi(l,j,k)*ksi_z(i,j,k)+dweta*eta_z(i,j,k)+dwphi(l,j,k)*phi_z(i,j,k))*costeta(l,j,k)  &
                       )*ijacob3(i,j,k) + wf(l,j,k)*ir(l,j,k) )
             rt(l,j,k) = vg(l,j,k)*( &
                       ((drksi(l,j,k)*ksi_x(i,j,k)+dreta*eta_x(i,j,k)+drphi(l,j,k)*phi_x(i,j,k))*sintcosp(l,j,k) &
                       +(drksi(l,j,k)*ksi_y(i,j,k)+dreta*eta_y(i,j,k)+drphi(l,j,k)*phi_y(i,j,k))*sintsinp(l,j,k) &
                       +(drksi(l,j,k)*ksi_z(i,j,k)+dreta*eta_z(i,j,k)+drphi(l,j,k)*phi_z(i,j,k))*costeta(l,j,k)  &
    ...
    do k=nzmnghp1,nz
       ...
       do i=1,ngh
          do j=ndy,nfy
             ...
             pt(i,j,l) = vg(i,j,l)*( &
                       ((dpksi(i,j,l)*ksi_x(i,j,k)+dpeta*eta_x(i,j,k)+dpphi(i,j,l)*phi_x(i,j,k))*sintcosp(i,j,l) &
                       +(dpksi(i,j,l)*ksi_y(i,j,k)+dpeta*eta_y(i,j,k)+dpphi(i,j,l)*phi_y(i,j,k))*sintsinp(i,j,l) &
                       +(dpksi(i,j,l)*ksi_z(i,j,k)+dpeta*eta_z(i,j,k)+dpphi(i,j,l)*phi_z(i,j,k))*costeta(i,j,l)  &
                       )*ijacob3(i,j,k) + pf(i,j,l)*ir(i,j,l) )
             ut(i,j,l) = vg(i,j,l)*( &
                       ((duksi(i,j,l)*ksi_x(i,j,k)+dueta*eta_x(i,j,k)+duphi(i,j,l)*phi_x(i,j,k))*sintcosp(i,j,l) &
                       +(duksi(i,j,l)*ksi_y(i,j,k)+dueta*eta_y(i,j,k)+duphi(i,j,l)*phi_y(i,j,k))*sintsinp(i,j,l) &
                       +(duksi(i,j,l)*ksi_z(i,j,k)+dueta*eta_z(i,j,k)+duphi(i,j,l)*phi_z(i,j,k))*costeta(i,j,l)  &
                       )*ijacob3(i,j,k) + uf(i,j,l)*ir(i,j,l) )
             vt(i,j,l) = vg(i,j,l)*( &
                       ((dvksi(i,j,l)*ksi_x(i,j,k)+dveta*eta_x(i,j,k)+dvphi(i,j,l)*phi_x(i,j,k))*sintcosp(i,j,l) &
                       +(dvksi(i,j,l)*ksi_y(i,j,k)+dveta*eta_y(i,j,k)+dvphi(i,j,l)*phi_y(i,j,k))*sintsinp(i,j,l) &
                       +(dvksi(i,j,l)*ksi_z(i,j,k)+dveta*eta_z(i,j,k)+dvphi(i,j,l)*phi_z(i,j,k))*costeta(i,j,l)  &
                       )*ijacob3(i,j,k) + vf(i,j,l)*ir(i,j,l) )
             wt(i,j,l) = vg(i,j,l)*( &
                       ((dwksi(i,j,l)*ksi_x(i,j,k)+dweta*eta_x(i,j,k)+dwphi(i,j,l)*phi_x(i,j,k))*sintcosp(i,j,l) &
                       +(dwksi(i,j,l)*ksi_y(i,j,k)+dweta*eta_y(i,j,k)+dwphi(i,j,l)*phi_y(i,j,k))*sintsinp(i,j,l) &
                       +(dwksi(i,j,l)*ksi_z(i,j,k)+dweta*eta_z(i,j,k)+dwphi(i,j,l)*phi_z(i,j,k))*costeta(i,j,l)  &
                       )*ijacob3(i,j,k) + wf(i,j,l)*ir(i,j,l) )
             rt(i,j,l) = vg(i,j,l)*( &
                       ((drksi(i,j,l)*ksi_x(i,j,k)+dreta*eta_x(i,j,k)+drphi(i,j,l)*phi_x(i,j,k))*sintcosp(i,j,l) &
                       +(drksi(i,j,l)*ksi_y(i,j,k)+dreta*eta_y(i,j,k)+drphi(i,j,l)*phi_y(i,j,k))*sintsinp(i,j,l) &
                       +(drksi(i,j,l)*ksi_z(i,j,k)+dreta*eta_z(i,j,k)+drphi(i,j,l)*phi_z(i,j,k))*costeta(i,j,l)  &
  ...
  !===============================================================================
    ...
    do k=nzmnghp1,nz
       ...
       do i=nxmnghp1,nx
          l=i-nxmngh
          do j=ndy,nfy
             dueta= a7(1)*(uf(l,j+1,m)-uf(l,j-1,m)) &
                  + a7(2)*(uf(l,j+2,m)-uf(l,j-2,m)) &
                  + a7(3)*(uf(l,j+3,m)-uf(l,j-3,m))
             dveta= a7(1)*(vf(l,j+1,m)-vf(l,j-1,m)) &
                  + a7(2)*(vf(l,j+2,m)-vf(l,j-2,m)) &
                  + a7(3)*(vf(l,j+3,m)-vf(l,j-3,m))
             dweta= a7(1)*(wf(l,j+1,m)-wf(l,j-1,m)) &
                  + a7(2)*(wf(l,j+2,m)-wf(l,j-2,m)) &
                  + a7(3)*(wf(l,j+3,m)-wf(l,j-3,m))
             dpeta= a7(1)*(pf(l,j+1,m)-pf(l,j-1,m)) &
                  + a7(2)*(pf(l,j+2,m)-pf(l,j-2,m)) &
                  + a7(3)*(pf(l,j+3,m)-pf(l,j-3,m))
             dreta= a7(1)*(rf(l,j+1,m)-rf(l,j-1,m)) &
                  + a7(2)*(rf(l,j+2,m)-rf(l,j-2,m)) &
                  + a7(3)*(rf(l,j+3,m)-rf(l,j-3,m))

             pt(l,j,m) = vg(l,j,m)*( &
                       ((dpksi(l,j,m)*ksi_x(i,j,k)+dpeta*eta_x(i,j,k)+dpphi(l,j,m)*phi_x(i,j,k))*sintcosp(l,j,m) &
                       +(dpksi(l,j,m)*ksi_y(i,j,k)+dpeta*eta_y(i,j,k)+dpphi(l,j,m)*phi_y(i,j,k))*sintsinp(l,j,m) &
                       +(dpksi(l,j,m)*ksi_z(i,j,k)+dpeta*eta_z(i,j,k)+dpphi(l,j,m)*phi_z(i,j,k))*costeta(l,j,m)  &
                       )*ijacob3(i,j,k) + pf(l,j,m)*ir(l,j,m) )
             ut(l,j,m) = vg(l,j,m)*( &
                       ((duksi(l,j,m)*ksi_x(i,j,k)+dueta*eta_x(i,j,k)+duphi(l,j,m)*phi_x(i,j,k))*sintcosp(l,j,m) &
                       +(duksi(l,j,m)*ksi_y(i,j,k)+dueta*eta_y(i,j,k)+duphi(l,j,m)*phi_y(i,j,k))*sintsinp(l,j,m) &
                       +(duksi(l,j,m)*ksi_z(i,j,k)+dueta*eta_z(i,j,k)+duphi(l,j,m)*phi_z(i,j,k))*costeta(l,j,m)  &
                       )*ijacob3(i,j,k) + uf(l,j,m)*ir(l,j,m) )
             vt(l,j,m) = vg(l,j,m)*( &
                       ((dvksi(l,j,m)*ksi_x(i,j,k)+dveta*eta_x(i,j,k)+dvphi(l,j,m)*phi_x(i,j,k))*sintcosp(l,j,m) &
                       +(dvksi(l,j,m)*ksi_y(i,j,k)+dveta*eta_y(i,j,k)+dvphi(l,j,m)*phi_y(i,j,k))*sintsinp(l,j,m) &
                       +(dvksi(l,j,m)*ksi_z(i,j,k)+dveta*eta_z(i,j,k)+dvphi(l,j,m)*phi_z(i,j,k))*costeta(l,j,m)  &
                       )*ijacob3(i,j,k) + vf(l,j,m)*ir(l,j,m) )
             wt(l,j,m) = vg(l,j,m)*( &
                       ((dwksi(l,j,m)*ksi_x(i,j,k)+dweta*eta_x(i,j,k)+dwphi(l,j,m)*phi_x(i,j,k))*sintcosp(l,j,m) &
                       +(dwksi(l,j,m)*ksi_y(i,j,k)+dweta*eta_y(i,j,k)+dwphi(l,j,m)*phi_y(i,j,k))*sintsinp(l,j,m) &
                       +(dwksi(l,j,m)*ksi_z(i,j,k)+dweta*eta_z(i,j,k)+dwphi(l,j,m)*phi_z(i,j,k))*costeta(l,j,m)  &
                       )*ijacob3(i,j,k) + wf(l,j,m)*ir(l,j,m) )
             rt(l,j,m) = vg(l,j,m)*( &
                       ((drksi(l,j,m)*ksi_x(i,j,k)+dreta*eta_x(i,j,k)+drphi(l,j,m)*phi_x(i,j,k))*sintcosp(l,j,m) &
                       +(drksi(l,j,m)*ksi_y(i,j,k)+dreta*eta_y(i,j,k)+drphi(l,j,m)*phi_y(i,j,k))*sintsinp(l,j,m) &
                       +(drksi(l,j,m)*ksi_z(i,j,k)+dreta*eta_z(i,j,k)+drphi(l,j,m)*phi_z(i,j,k))*costeta(l,j,m)  &
...
end submodule smod_TamDong3d_edgey_c3
