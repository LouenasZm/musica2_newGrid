!=================================================================================
submodule (mod_flux_euler) smod_flux_euler_5pts
!=================================================================================
  !> author: XG
  !> date: January 2024
  !> Cartesian version - compute Eulerian fluxes (inviscid part) with 5-pt stencil
!=================================================================================
       ...
       do j=ndy_e,nfy_e
          do i=ndx,nfx
             ...
             Krho(i,j,k)  = ( a5(1) * ( Frho(i+1,j,k)-Frho(i-1,j,k) )   &
                            + a5(2) * ( Frho(i+2,j,k)-Frho(i-2,j,k) ) ) *idx(i)

             Krhou(i,j,k) = ( a5(1) * ( Frhou(i+1,j,k)-Frhou(i-1,j,k) ) &
                            + a5(2) * ( Frhou(i+2,j,k)-Frhou(i-2,j,k) ) ) *idx(i)

             Krhov(i,j,k) = ( a5(1) * ( Frhov(i+1,j,k)-Frhov(i-1,j,k) ) &
                            + a5(2) * ( Frhov(i+2,j,k)-Frhov(i-2,j,k) ) ) *idx(i)

             Krhow(i,j,k) = ( a5(1) * ( Frhow(i+1,j,k)-Frhow(i-1,j,k) ) &
                            + a5(2) * ( Frhow(i+2,j,k)-Frhow(i-2,j,k) ) ) *idx(i)

             Krhoe(i,j,k) = ( a5(1) * ( Frhoe(i+1,j,k)-Frhoe(i-1,j,k) ) &
                            + a5(2) * ( Frhoe(i+2,j,k)-Frhoe(i-2,j,k) ) ) *idx(i)
          enddo
       enddo
    enddo
       ...
       do j=ndy,nfy
          do i=ndx_e,nfx_e
             Krho(i,j,k)  = ( a5(1) * ( Frho(i,j+1,k)-Frho(i,j-1,k) ) &
                            + a5(2) * ( Frho(i,j+2,k)-Frho(i,j-2,k) ) ) *idy(j) &
                            + Krho(i,j,k)

             Krhou(i,j,k) = ( a5(1) * ( Frhou(i,j+1,k)-Frhou(i,j-1,k) ) &
                            + a5(2) * ( Frhou(i,j+2,k)-Frhou(i,j-2,k) ) ) *idy(j) &
                            + Krhou(i,j,k)

             Krhov(i,j,k) = ( a5(1) * ( Frhov(i,j+1,k)-Frhov(i,j-1,k) ) &
                            + a5(2) * ( Frhov(i,j+2,k)-Frhov(i,j-2,k) ) ) *idy(j) &
                            + Krhov(i,j,k)

             Krhow(i,j,k) = ( a5(1) * ( Frhow(i,j+1,k)-Frhow(i,j-1,k) ) &
                            + a5(2) * ( Frhow(i,j+2,k)-Frhow(i,j-2,k) ) ) *idy(j) &
                            + Krhow(i,j,k)

             Krhoe(i,j,k) = ( a5(1) * ( Frhoe(i,j+1,k)-Frhoe(i,j-1,k) ) &
                            + a5(2) * ( Frhoe(i,j+2,k)-Frhoe(i,j-2,k) ) ) *idy(j) &
                            + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,k)  = ( a5(1)* ( Frho(i,j,k+1)-Frho(i,j,k-1) ) &
                            + a5(2)* ( Frho(i,j,k+2)-Frho(i,j,k-2) ) ) *idz(k) &
                            + Krho(i,j,k)

             Krhou(i,j,k) = ( a5(1)* ( Frhou(i,j,k+1)-Frhou(i,j,k-1) ) &
                            + a5(2)* ( Frhou(i,j,k+2)-Frhou(i,j,k-2)  ) ) *idz(k) &
                            + Krhou(i,j,k)

             Krhov(i,j,k) = ( a5(1)* ( Frhov(i,j,k+1)-Frhov(i,j,k-1) ) &
                            + a5(2)* ( Frhov(i,j,k+2)-Frhov(i,j,k-2) ) ) *idz(k) &
                            + Krhov(i,j,k)

             Krhow(i,j,k) = ( a5(1)* ( Frhow(i,j,k+1)-Frhow(i,j,k-1) ) &
                            + a5(2)* ( Frhow(i,j,k+2)-Frhow(i,j,k-2) ) ) *idz(k) &
                            + Krhow(i,j,k)

             Krhoe(i,j,k) = ( a5(1)* ( Frhoe(i,j,k+1)-Frhoe(i,j,k-1) ) &
                            + a5(2)* ( Frhoe(i,j,k+2)-Frhoe(i,j,k-2) ) ) *idz(k) &
                            + Krhoe(i,j,k)
          enddo
       enddo
    enddo
...
end submodule smod_flux_euler_5pts
