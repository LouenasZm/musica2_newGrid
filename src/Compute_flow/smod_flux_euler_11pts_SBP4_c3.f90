!=================================================================================
submodule (mod_flux_euler) smod_flux_euler_11pts_SBP4_c3
!=================================================================================
  !> author: XG
  !> date: January 2024
  !> Full 3D curvilinear Cartesian version - compute Eulerian fluxes (inviscid part) with 11-pt stencil
...
contains
  ...
  !==============================================================================
    ...
    !> - Full 3D curvilinear curvilinear version with SBP4 boundary schemes -
  !==============================================================================
    implicit none
    ! ---------------------------------------------------------------------------
    integer :: i,j,k
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,1) = as4p0(1)* Frho(i,j,1)+as4p0(2)* Frho(i,j,2) &
                         + as4p0(3)* Frho(i,j,3)+as4p0(4)* Frho(i,j,4) + Krho(i,j,1)
             Krhou(i,j,1)= as4p0(1)*Frhou(i,j,1)+as4p0(2)*Frhou(i,j,2) &
                         + as4p0(3)*Frhou(i,j,3)+as4p0(4)*Frhou(i,j,4) + Krhou(i,j,1)
             Krhov(i,j,1)= as4p0(1)*Frhov(i,j,1)+as4p0(2)*Frhov(i,j,2) &
                         + as4p0(3)*Frhov(i,j,3)+as4p0(4)*Frhov(i,j,4) + Krhov(i,j,1)
             Krhow(i,j,1)= as4p0(1)*Frhow(i,j,1)+as4p0(2)*Frhow(i,j,2) &
                         + as4p0(3)*Frhow(i,j,3)+as4p0(4)*Frhow(i,j,4) + Krhow(i,j,1)
             Krhoe(i,j,1)= as4p0(1)*Frhoe(i,j,1)+as4p0(2)*Frhoe(i,j,2) &
                         + as4p0(3)*Frhoe(i,j,3)+as4p0(4)*Frhoe(i,j,4) + Krhoe(i,j,1)
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,2) = as4p1(1)* Frho(i,j,1)+as4p1(2)* Frho(i,j,2) &
                         + as4p1(3)* Frho(i,j,3)+as4p1(4)* Frho(i,j,4) + Krho(i,j,2)
             Krhou(i,j,2)= as4p1(1)*Frhou(i,j,1)+as4p1(2)*Frhou(i,j,2) &
                         + as4p1(3)*Frhou(i,j,3)+as4p1(4)*Frhou(i,j,4) + Krhou(i,j,2)
             Krhov(i,j,2)= as4p1(1)*Frhov(i,j,1)+as4p1(2)*Frhov(i,j,2) &
                         + as4p1(3)*Frhov(i,j,3)+as4p1(4)*Frhov(i,j,4) + Krhov(i,j,2)
             Krhow(i,j,2)= as4p1(1)*Frhow(i,j,1)+as4p1(2)*Frhow(i,j,2) &
                         + as4p1(3)*Frhow(i,j,3)+as4p1(4)*Frhow(i,j,4) + Krhow(i,j,2)
             Krhoe(i,j,2)= as4p1(1)*Frhoe(i,j,1)+as4p1(2)*Frhoe(i,j,2) &
                         + as4p1(3)*Frhoe(i,j,3)+as4p1(4)*Frhoe(i,j,4) + Krhoe(i,j,2)
          enddo
       enddo
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,k) = a11(1)* ( Frho(i,j,k+1)-Frho(i,j,k-1) ) &
                         ...
                         + a11(5)* ( Frho(i,j,k+5)-Frho(i,j,k-5) ) &
                         + Krho(i,j,k)
             ...
             Krhou(i,j,k)= a11(1)* ( Frhou(i,j,k+1)-Frhou(i,j,k-1) ) &
                         ...
                         + a11(5)* ( Frhou(i,j,k+5)-Frhou(i,j,k-5) ) &
                         + Krhou(i,j,k)
             ...
             Krhov(i,j,k)= a11(1)* ( Frhov(i,j,k+1)-Frhov(i,j,k-1) ) &
                         ...
                         + a11(5)* ( Frhov(i,j,k+5)-Frhov(i,j,k-5) ) &
                         + Krhov(i,j,k)
             ...
             Krhow(i,j,k)= a11(1)* ( Frhow(i,j,k+1)-Frhow(i,j,k-1) ) &
                         ...
                         + a11(5)* ( Frhow(i,j,k+5)-Frhow(i,j,k-5) ) &
                         + Krhow(i,j,k)
             ...
             Krhoe(i,j,k)= a11(1)* ( Frhoe(i,j,k+1)-Frhoe(i,j,k-1) ) &
                         ...
                         + a11(5)* ( Frhoe(i,j,k+5)-Frhoe(i,j,k-5) ) &
                         + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,k) = as4m1(1)* Frho(i,j,nz  )+as4m1(2)* Frho(i,j,nz-1) &
                         + as4m1(3)* Frho(i,j,nz-2)+as4m1(4)* Frho(i,j,nz-3) + Krho(i,j,k)
             Krhou(i,j,k)= as4m1(1)*Frhou(i,j,nz  )+as4m1(2)*Frhou(i,j,nz-1) &
                         + as4m1(3)*Frhou(i,j,nz-2)+as4m1(4)*Frhou(i,j,nz-3) + Krhou(i,j,k)
             Krhov(i,j,k)= as4m1(1)*Frhov(i,j,nz  )+as4m1(2)*Frhov(i,j,nz-1) &
                         + as4m1(3)*Frhov(i,j,nz-2)+as4m1(4)*Frhov(i,j,nz-3) + Krhov(i,j,k)
             Krhow(i,j,k)= as4m1(1)*Frhow(i,j,nz  )+as4m1(2)*Frhow(i,j,nz-1) &
                         + as4m1(3)*Frhow(i,j,nz-2)+as4m1(4)*Frhow(i,j,nz-3) + Krhow(i,j,k)
             Krhoe(i,j,k)= as4m1(1)*Frhoe(i,j,nz  )+as4m1(2)*Frhoe(i,j,nz-1) &
                         + as4m1(3)*Frhoe(i,j,nz-2)+as4m1(4)*Frhoe(i,j,nz-3) + Krhoe(i,j,k)
          enddo
       enddo
    endif
       ...
       do j=ndy_e,nfy_e
          do i=ndx_e,nfx_e
             Krho(i,j,k) = as4m0(1)* Frho(i,j,nz  )+as4m0(2)* Frho(i,j,nz-1) &
                         + as4m0(3)* Frho(i,j,nz-2)+as4m0(4)* Frho(i,j,nz-3) + Krho(i,j,k)
             Krhou(i,j,k)= as4m0(1)*Frhou(i,j,nz  )+as4m0(2)*Frhou(i,j,nz-1) &
                         + as4m0(3)*Frhou(i,j,nz-2)+as4m0(4)*Frhou(i,j,nz-3) + Krhou(i,j,k)
             Krhov(i,j,k)= as4m0(1)*Frhov(i,j,nz  )+as4m0(2)*Frhov(i,j,nz-1) &
                         + as4m0(3)*Frhov(i,j,nz-2)+as4m0(4)*Frhov(i,j,nz-3) + Krhov(i,j,k)
             Krhow(i,j,k)= as4m0(1)*Frhow(i,j,nz  )+as4m0(2)*Frhow(i,j,nz-1) &
                         + as4m0(3)*Frhow(i,j,nz-2)+as4m0(4)*Frhow(i,j,nz-3) + Krhow(i,j,k)
             Krhoe(i,j,k)= as4m0(1)*Frhoe(i,j,nz  )+as4m0(2)*Frhoe(i,j,nz-1) &
                         + as4m0(3)*Frhoe(i,j,nz-2)+as4m0(4)*Frhoe(i,j,nz-3) + Krhoe(i,j,k)
          enddo
       enddo
    endif
...
end submodule smod_flux_euler_11pts_SBP4_c3
