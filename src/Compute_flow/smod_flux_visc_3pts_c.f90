!=================================================================================
submodule (mod_flux_visc) smod_flux_visc_3pts_c
!=================================================================================
  !> author: XG
  !> date: January 2024
  !> 2.5D curvilinear version - compute viscous fluxes with 3-pt stencil
!=================================================================================
...
contains
  ...
  !==============================================================================
    ...
    ! ---------------------------------------------------------------------------
    real(wp), dimension(ndxt_v:nfxt_v,ndyt_v:nfyt_v,ndzt_v:nfzt_v) :: tauR11,tauR12,tauR13,tauR22,tauR23,tauR33,cokt
    ...
    if (.not.is_RANS) then
       ...
       !             Frhow(i,j,k)= tau13*y_eta_v(i,j)-tau23*x_eta_v(i,j)
       !             Frhoe(i,j,k)= (uu(i,j,k)*Frhou(i,j,k)+vv(i,j,k)*Frhov(i,j,k)+ww(i,j,k)*Frhow(i,j,k)) &
       !                             -(cok(i,j,k)+cokt)*(dTx(i,j,k)*y_eta_v(i,j)-dTy(i,j,k)*x_eta_v(i,j))
       ...
       !             Grhow(i,j,k)=-tau13*y_ksi_v(i,j)+tau23*x_ksi_v(i,j)
       !             Grhoe(i,j,k)= (uu(i,j,k)*Grhou(i,j,k)+vv(i,j,k)*Grhov(i,j,k)+ww(i,j,k)*Grhow(i,j,k)) &
       !                             -(cok(i,j,k)+cokt)*(-dTx(i,j,k)*y_ksi_v(i,j)+dTy(i,j,k)*x_ksi_v(i,j))
       ...
       do k=ndz_v,nfz_v
          do j=ndy_v,nfy_v
             Krhou(i,j,k) = ( a02(1)*Frhou(i,j,k) + a02(2)*Frhou(i+1,j,k) &
                            + a02(3)*Frhou(i+2,j,k))*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a02(1)*Frhov(i,j,k) + a02(2)*Frhov(i+1,j,k) &
                            + a02(3)*Frhov(i+2,j,k))*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a02(1)*Frhow(i,j,k) + a02(2)*Frhow(i+1,j,k) &
                            + a02(3)*Frhow(i+2,j,k))*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a02(1)*Frhoe(i,j,k) + a02(2)*Frhoe(i+1,j,k) &
                            + a02(3)*Frhoe(i+2,j,k))*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    endif
    ...
    do k=ndz_v,nfz_v
       do j=ndy_v,nfy_v
          do i=ndx_vi,nfx_vi
             Krhou(i,j,k) = a3(1)*( Frhou(i+1,j,k)-Frhou(i-1,j,k) )*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = a3(1)*( Frhov(i+1,j,k)-Frhov(i-1,j,k) )*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = a3(1)*( Frhow(i+1,j,k)-Frhow(i-1,j,k) )*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = a3(1)*( Frhoe(i+1,j,k)-Frhoe(i-1,j,k) )*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do k=ndz_v,nfz_v
          do j=ndy_v,nfy_v
             Krhou(i,j,k) = ( a20(1)*Frhou(i,j,k) + a20(2)*Frhou(i-1,j,k) &
                            + a20(3)*Frhou(i-2,j,k))*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a20(1)*Frhov(i,j,k) + a20(2)*Frhov(i-1,j,k) &
                            + a20(3)*Frhov(i-2,j,k))*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a20(1)*Frhow(i,j,k) + a20(2)*Frhow(i-1,j,k) &
                            + a20(3)*Frhow(i-2,j,k))*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a20(1)*Frhoe(i,j,k) + a20(2)*Frhoe(i-1,j,k) &
                            + a20(3)*Frhoe(i-2,j,k))*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    endif
       ...
       do k=ndz_v,nfz_v
          do i=ndx_v,nfx_v
             Krhou(i,j,k) = ( a02(1)*Grhou(i,j,k) + a02(2)*Grhou(i,j+1,k) &
                            + a02(3)*Grhou(i,j+2,k))*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a02(1)*Grhov(i,j,k) + a02(2)*Grhov(i,j+1,k) &
                            + a02(3)*Grhov(i,j+2,k))*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a02(1)*Grhow(i,j,k) + a02(2)*Grhow(i,j+1,k) &
                            + a02(3)*Grhow(i,j+2,k))*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a02(1)*Grhoe(i,j,k) + a02(2)*Grhoe(i,j+1,k) &
                            + a02(3)*Grhoe(i,j+2,k))*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    endif
    ...
    do k=ndz_v,nfz_v
       do j=ndy_vi,nfy_vi
          do i=ndx_v,nfx_v
             Krhou(i,j,k) = a3(1)*( Grhou(i,j+1,k)-Grhou(i,j-1,k) )*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = a3(1)*( Grhov(i,j+1,k)-Grhov(i,j-1,k) )*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = a3(1)*( Grhow(i,j+1,k)-Grhow(i,j-1,k) )*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = a3(1)*( Grhoe(i,j+1,k)-Grhoe(i,j-1,k) )*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do k=ndz_v,nfz_v
          do i=ndx_v,nfx_v
             Krhou(i,j,k) = ( a20(1)*Grhou(i,j,k) + a20(2)*Grhou(i,j-1,k) &
                            + a20(3)*Grhou(i,j-2,k))*ijacob_v(i,j) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a20(1)*Grhov(i,j,k) + a20(2)*Grhov(i,j-1,k) &
                            + a20(3)*Grhov(i,j-2,k))*ijacob_v(i,j) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a20(1)*Grhow(i,j,k) + a20(2)*Grhow(i,j-1,k) &
                            + a20(3)*Grhow(i,j-2,k))*ijacob_v(i,j) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a20(1)*Grhoe(i,j,k) + a20(2)*Grhoe(i,j-1,k) &
                            + a20(3)*Grhoe(i,j-2,k))*ijacob_v(i,j) + Krhoe(i,j,k)
          enddo
       enddo
    endif
...
end submodule smod_flux_visc_3pts_c
