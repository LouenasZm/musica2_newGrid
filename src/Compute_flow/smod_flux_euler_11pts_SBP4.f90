!=================================================================================
submodule (mod_flux_euler) smod_flux_euler_11pts_SBP4
!=================================================================================
  !> author: XG
  !> date: January 2024
...
contains
  ...
  !==============================================================================
    ...
    !> - Cartesian version with SBP4 boundary schemes
  !==============================================================================
    !use mod_fluid ! DEV TEMP
    implicit none
    ! ---------------------------------------------------------------------------
    integer :: i,j,k
    !real(wp) :: h11 ! DEV TEMP
    !real(wp) :: drdy,dudy,dvdy,dpdy,vq ! DEV TEMP
    ! ---------------------------------------------------------------------------
       ...
       do k=ndz_e,nfz_e
          do j=ndy_e,nfy_e
             Krho(1,j,k) = (as4p0(1)* Frho(1,j,k)+as4p0(2)* Frho(2,j,k) &
                          + as4p0(3)* Frho(3,j,k)+as4p0(4)* Frho(4,j,k))*idx1_imin
             Krhou(1,j,k)= (as4p0(1)*Frhou(1,j,k)+as4p0(2)*Frhou(2,j,k) &
                          + as4p0(3)*Frhou(3,j,k)+as4p0(4)*Frhou(4,j,k))*idx1_imin
             Krhov(1,j,k)= (as4p0(1)*Frhov(1,j,k)+as4p0(2)*Frhov(2,j,k) &
                          + as4p0(3)*Frhov(3,j,k)+as4p0(4)*Frhov(4,j,k))*idx1_imin
             Krhow(1,j,k)= (as4p0(1)*Frhow(1,j,k)+as4p0(2)*Frhow(2,j,k) &
                          + as4p0(3)*Frhow(3,j,k)+as4p0(4)*Frhow(4,j,k))*idx1_imin
             Krhoe(1,j,k)= (as4p0(1)*Frhoe(1,j,k)+as4p0(2)*Frhoe(2,j,k) &
                          + as4p0(3)*Frhoe(3,j,k)+as4p0(4)*Frhoe(4,j,k))*idx1_imin
          enddo
       enddo
    endif
       ...
       do k=ndz_e,nfz_e
          do j=ndy_e,nfy_e
             Krho(2,j,k) = (as4p1(1)* Frho(1,j,k)+as4p1(2)* Frho(2,j,k) &
                          + as4p1(3)* Frho(3,j,k)+as4p1(4)* Frho(4,j,k))*idx2_imin
             Krhou(2,j,k)= (as4p1(1)*Frhou(1,j,k)+as4p1(2)*Frhou(2,j,k) &
                          + as4p1(3)*Frhou(3,j,k)+as4p1(4)*Frhou(4,j,k))*idx2_imin
             Krhov(2,j,k)= (as4p1(1)*Frhov(1,j,k)+as4p1(2)*Frhov(2,j,k) &
                          + as4p1(3)*Frhov(3,j,k)+as4p1(4)*Frhov(4,j,k))*idx2_imin
             Krhow(2,j,k)= (as4p1(1)*Frhow(1,j,k)+as4p1(2)*Frhow(2,j,k) &
                          + as4p1(3)*Frhow(3,j,k)+as4p1(4)*Frhow(4,j,k))*idx2_imin
             Krhoe(2,j,k)= (as4p1(1)*Frhoe(1,j,k)+as4p1(2)*Frhoe(2,j,k) &
                          + as4p1(3)*Frhoe(3,j,k)+as4p1(4)*Frhoe(4,j,k))*idx2_imin
          enddo
       enddo
       ...
       do k=ndz_e,nfz_e
          do j=ndy_e,nfy_e
             Krho(i,j,k) = (as4m1(1)* Frho(nx  ,j,k)+as4m1(2)* Frho(nx-1,j,k) &
                          + as4m1(3)* Frho(nx-2,j,k)+as4m1(4)* Frho(nx-3,j,k))*idx2_imax
             Krhou(i,j,k)= (as4m1(1)*Frhou(nx  ,j,k)+as4m1(2)*Frhou(nx-1,j,k) &
                          + as4m1(3)*Frhou(nx-2,j,k)+as4m1(4)*Frhou(nx-3,j,k))*idx2_imax
             Krhov(i,j,k)= (as4m1(1)*Frhov(nx  ,j,k)+as4m1(2)*Frhov(nx-1,j,k) &
                          + as4m1(3)*Frhov(nx-2,j,k)+as4m1(4)*Frhov(nx-3,j,k))*idx2_imax
             Krhow(i,j,k)= (as4m1(1)*Frhow(nx  ,j,k)+as4m1(2)*Frhow(nx-1,j,k) &
                          + as4m1(3)*Frhow(nx-2,j,k)+as4m1(4)*Frhow(nx-3,j,k))*idx2_imax
             Krhoe(i,j,k)= (as4m1(1)*Frhoe(nx  ,j,k)+as4m1(2)*Frhoe(nx-1,j,k) &
                          + as4m1(3)*Frhoe(nx-2,j,k)+as4m1(4)*Frhoe(nx-3,j,k))*idx2_imax
          enddo
       enddo
    endif
       ...
       do k=ndz_e,nfz_e
          do j=ndy_e,nfy_e
             Krho(i,j,k) = (as4m0(1)* Frho(nx  ,j,k)+as4m0(2)* Frho(nx-1,j,k) &
                          + as4m0(3)* Frho(nx-2,j,k)+as4m0(4)* Frho(nx-3,j,k))*idx1_imax
             Krhou(i,j,k)= (as4m0(1)*Frhou(nx  ,j,k)+as4m0(2)*Frhou(nx-1,j,k) &
                          + as4m0(3)*Frhou(nx-2,j,k)+as4m0(4)*Frhou(nx-3,j,k))*idx1_imax
             Krhov(i,j,k)= (as4m0(1)*Frhov(nx  ,j,k)+as4m0(2)*Frhov(nx-1,j,k) &
                          + as4m0(3)*Frhov(nx-2,j,k)+as4m0(4)*Frhov(nx-3,j,k))*idx1_imax
             Krhow(i,j,k)= (as4m0(1)*Frhow(nx  ,j,k)+as4m0(2)*Frhow(nx-1,j,k) &
                          + as4m0(3)*Frhow(nx-2,j,k)+as4m0(4)*Frhow(nx-3,j,k))*idx1_imax
             Krhoe(i,j,k)= (as4m0(1)*Frhoe(nx  ,j,k)+as4m0(2)*Frhoe(nx-1,j,k) &
                          + as4m0(3)*Frhoe(nx-2,j,k)+as4m0(4)*Frhoe(nx-3,j,k))*idx1_imax
          enddo
       enddo
    endif
       ...
       do k=ndz_e,nfz_e
          do i=ndx_e,nfx_e          
             Krho(i,1,k) = (as4p0(1)* Frho(i,1,k)+as4p0(2)* Frho(i,2,k) &
                          + as4p0(3)* Frho(i,3,k)+as4p0(4)* Frho(i,4,k))*idy1_jmin + Krho(i,1,k)
             Krhou(i,1,k)= (as4p0(1)*Frhou(i,1,k)+as4p0(2)*Frhou(i,2,k) &
                          + as4p0(3)*Frhou(i,3,k)+as4p0(4)*Frhou(i,4,k))*idy1_jmin + Krhou(i,1,k)
             Krhov(i,1,k)= (as4p0(1)*Frhov(i,1,k)+as4p0(2)*Frhov(i,2,k) &
                          + as4p0(3)*Frhov(i,3,k)+as4p0(4)*Frhov(i,4,k))*idy1_jmin + Krhov(i,1,k)
                          !+0.5_wp/h11*idy1_jmin*vv(i,1,k)*(vv(i,1,k)+c_(i,1,k))
             Krhow(i,1,k)= (as4p0(1)*Frhow(i,1,k)+as4p0(2)*Frhow(i,2,k) &
                          + as4p0(3)*Frhow(i,3,k)+as4p0(4)*Frhow(i,4,k))*idy1_jmin + Krhow(i,1,k)
             Krhoe(i,1,k)= (as4p0(1)*Frhoe(i,1,k)+as4p0(2)*Frhoe(i,2,k) &
                          + as4p0(3)*Frhoe(i,3,k)+as4p0(4)*Frhoe(i,4,k))*idy1_jmin + Krhoe(i,1,k)
...
!!$       do k=ndz_e,nfz_e
!!$          do i=ndx_e,nfx_e          
!!$             drdy= (as4p0(1)*rho_n(i,1,k)+as4p0(2)*rho_n(i,2,k) &
!!$                  + as4p0(3)*rho_n(i,3,k)+as4p0(4)*rho_n(i,4,k))*idy1_jmin
!!$             dudy= (as4p0(1)*uu(i,1,k)+as4p0(2)*uu(i,2,k) &
!!$                  + as4p0(3)*uu(i,3,k)+as4p0(4)*uu(i,4,k))*idy1_jmin
!!$             dvdy= (as4p0(1)*vv(i,1,k)+as4p0(2)*vv(i,2,k) &
!!$                  + as4p0(3)*vv(i,3,k)+as4p0(4)*vv(i,4,k))*idy1_jmin
!!$             dpdy= (as4p0(1)*prs(i,1,k)+as4p0(2)*prs(i,2,k) &
!!$                  + as4p0(3)*prs(i,3,k)+as4p0(4)*prs(i,4,k))*idy1_jmin
!!$
!!$             !vv(i,j,k)=0.0_wp
!!$             !dpdy=0.
!!$             Krho(i,j,k)=rho_n(i,j,k)*dvdy+vv(i,j,k)*drdy + Krho(i,j,k)
!!$             Krhou(i,j,k)=rhou_n(i,j,k)*dvdy+rhov_n(i,j,k)*dudy &
!!$                  + uu(i,j,k)*vv(i,j,k)*drdy + Krhou(i,j,k)
!!$             Krhov(i,j,k)=dpdy + vv(i,j,k)*vv(i,j,k)*drdy &
!!$                  + 2.0_wp*rhov_n(i,j,k)*dvdy + Krhov(i,j,k)
!!$             vq = (uu(i,j,k)**2+vv(i,j,k)**2)*0.5_wp
!!$             Krhoe(i,j,k)=gam/(gam-1.0_wp)*( vv(i,j,k)*dpdy + prs(i,j,k)*dvdy ) &
!!$                  + rhov_n(i,j,k)*( uu(i,j,k)*dudy + vv(i,j,k)*dvdy ) &
!!$                  + vq*( vv(i,j,k)*drdy + rho_n(i,j,k)*dvdy ) &
!!$                  + Krhoe(i,j,k)
!!$          enddo
!!$       enddo
       ...
       do k=ndz_e,nfz_e
          do i=ndx_e,nfx_e          
             Krho(i,2,k) = (as4p1(1)* Frho(i,1,k)+as4p1(2)* Frho(i,2,k) &
                          + as4p1(3)* Frho(i,3,k)+as4p1(4)* Frho(i,4,k))*idy2_jmin + Krho(i,2,k)
             Krhou(i,2,k)= (as4p1(1)*Frhou(i,1,k)+as4p1(2)*Frhou(i,2,k) &
                          + as4p1(3)*Frhou(i,3,k)+as4p1(4)*Frhou(i,4,k))*idy2_jmin + Krhou(i,2,k)
             Krhov(i,2,k)= (as4p1(1)*Frhov(i,1,k)+as4p1(2)*Frhov(i,2,k) &
                          + as4p1(3)*Frhov(i,3,k)+as4p1(4)*Frhov(i,4,k))*idy2_jmin + Krhov(i,2,k)
             Krhow(i,2,k)= (as4p1(1)*Frhow(i,1,k)+as4p1(2)*Frhow(i,2,k) &
                          + as4p1(3)*Frhow(i,3,k)+as4p1(4)*Frhow(i,4,k))*idy2_jmin + Krhow(i,2,k)
             Krhoe(i,2,k)= (as4p1(1)*Frhoe(i,1,k)+as4p1(2)*Frhoe(i,2,k) &
                          + as4p1(3)*Frhoe(i,3,k)+as4p1(4)*Frhoe(i,4,k))*idy2_jmin + Krhoe(i,2,k)
          enddo
       enddo
       ...
       do k=ndz_e,nfz_e
          do i=ndx_e,nfx_e
             Krho(i,j,k) = (as4m1(1)* Frho(i,ny  ,k)+as4m1(2)* Frho(i,ny-1,k) &
                          + as4m1(3)* Frho(i,ny-2,k)+as4m1(4)* Frho(i,ny-3,k))*idy2_jmax + Krho(i,j,k)
             Krhou(i,j,k)= (as4m1(1)*Frhou(i,ny  ,k)+as4m1(2)*Frhou(i,ny-1,k) &
                          + as4m1(3)*Frhou(i,ny-2,k)+as4m1(4)*Frhou(i,ny-3,k))*idy2_jmax + Krhou(i,j,k)
             Krhov(i,j,k)= (as4m1(1)*Frhov(i,ny  ,k)+as4m1(2)*Frhov(i,ny-1,k) &
                          + as4m1(3)*Frhov(i,ny-2,k)+as4m1(4)*Frhov(i,ny-3,k))*idy2_jmax + Krhov(i,j,k)
             Krhov(i,j,k)= (as4m1(1)*Frhov(i,ny  ,k)+as4m1(2)*Frhov(i,ny-1,k) &
                          + as4m1(3)*Frhov(i,ny-2,k)+as4m1(4)*Frhov(i,ny-3,k))*idy2_jmax + Krhov(i,j,k)
             Krhoe(i,j,k)= (as4m1(1)*Frhoe(i,ny  ,k)+as4m1(2)*Frhoe(i,ny-1,k) &
                          + as4m1(3)*Frhoe(i,ny-2,k)+as4m1(4)*Frhoe(i,ny-3,k))*idy2_jmax + Krhoe(i,j,k)
          enddo
       enddo
    endif
       ...
       do k=ndz_e,nfz_e
          do i=ndx_e,nfx_e
             Krho(i,j,k) = (as4m0(1)* Frho(i,ny  ,k)+as4m0(2)* Frho(i,ny-1,k) &
                          + as4m0(3)* Frho(i,ny-2,k)+as4m0(4)* Frho(i,ny-3,k))*idy1_jmax + Krho(i,j,k)
             Krhou(i,j,k)= (as4m0(1)*Frhou(i,ny  ,k)+as4m0(2)*Frhou(i,ny-1,k) &
                          + as4m0(3)*Frhou(i,ny-2,k)+as4m0(4)*Frhou(i,ny-3,k))*idy1_jmax + Krhou(i,j,k)
             Krhov(i,j,k)= (as4m0(1)*Frhov(i,ny  ,k)+as4m0(2)*Frhov(i,ny-1,k) &
                          + as4m0(3)*Frhov(i,ny-2,k)+as4m0(4)*Frhov(i,ny-3,k))*idy1_jmax + Krhov(i,j,k)
             Krhov(i,j,k)= (as4m0(1)*Frhov(i,ny  ,k)+as4m0(2)*Frhov(i,ny-1,k) &
                          + as4m0(3)*Frhov(i,ny-2,k)+as4m0(4)*Frhov(i,ny-3,k))*idy1_jmax + Krhov(i,j,k)
             Krhoe(i,j,k)= (as4m0(1)*Frhoe(i,ny  ,k)+as4m0(2)*Frhoe(i,ny-1,k) &
                          + as4m0(3)*Frhoe(i,ny-2,k)+as4m0(4)*Frhoe(i,ny-3,k))*idy1_jmax + Krhoe(i,j,k)
          enddo
       enddo
    endif
...
end submodule smod_flux_euler_11pts_SBP4
