!=================================================================================
submodule (mod_flux_visc) smod_flux_visc_5pts_c3
!=================================================================================
  !> author: XG
  !> date: January 2024
  !> 3D curvilinear version - compute viscous fluxes with 5-pt stencil
!=================================================================================
       ...
       do j=ndy_v,nfy_v
          do i=ndx_vi,nfx_vi
             Krhou(i,j,k) = ( a5(1)*( Frhou(i+1,j,k)-Frhou(i-1,j,k) ) &
                            + a5(2)*( Frhou(i+2,j,k)-Frhou(i-2,j,k) ) )*ijacob3_v(i,j,k) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a5(1)*( Frhov(i+1,j,k)-Frhov(i-1,j,k) ) &
                            + a5(2)*( Frhov(i+2,j,k)-Frhov(i-2,j,k) ) )*ijacob3_v(i,j,k) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a5(1)*( Frhow(i+1,j,k)-Frhow(i-1,j,k) ) &
                            + a5(2)*( Frhow(i+2,j,k)-Frhow(i-2,j,k) ) )*ijacob3_v(i,j,k) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a5(1)*( Frhoe(i+1,j,k)-Frhoe(i-1,j,k) ) &
                            + a5(2)*( Frhoe(i+2,j,k)-Frhoe(i-2,j,k) ) )*ijacob3_v(i,j,k) + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do j=ndy_vi,nfy_vi
          do i=ndx_v,nfx_v
             Krhou(i,j,k) = ( a5(1)*( Grhou(i,j+1,k)-Grhou(i,j-1,k)) &
                            + a5(2)*( Grhou(i,j+2,k)-Grhou(i,j-2,k)) )*ijacob3_v(i,j,k) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a5(1)*( Grhov(i,j+1,k)-Grhov(i,j-1,k)) &
                            + a5(2)*( Grhov(i,j+2,k)-Grhov(i,j-2,k)) )*ijacob3_v(i,j,k) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a5(1)*( Grhow(i,j+1,k)-Grhow(i,j-1,k)) &
                            + a5(2)*( Grhow(i,j+2,k)-Grhow(i,j-2,k)) )*ijacob3_v(i,j,k) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a5(1)*( Grhoe(i,j+1,k)-Grhoe(i,j-1,k)) &
                            + a5(2)*( Grhoe(i,j+2,k)-Grhoe(i,j-2,k)) )*ijacob3_v(i,j,k) + Krhoe(i,j,k)
          enddo
       enddo
    enddo
       ...
       do j=ndy_v,nfy_v
          do i=ndx_v,nfx_v
             Krhou(i,j,k) = ( a5(1)*( Hrhou(i,j,k+1)-Hrhou(i,j,k-1)) &
                            + a5(2)*( Hrhou(i,j,k+2)-Hrhou(i,j,k-2)) )*ijacob3_v(i,j,k) + Krhou(i,j,k)
             Krhov(i,j,k) = ( a5(1)*( Hrhov(i,j,k+1)-Hrhov(i,j,k-1)) &
                            + a5(2)*( Hrhov(i,j,k+2)-Hrhov(i,j,k-2)) )*ijacob3_v(i,j,k) + Krhov(i,j,k)
             Krhow(i,j,k) = ( a5(1)*( Hrhow(i,j,k+1)-Hrhow(i,j,k-1)) &
                            + a5(2)*( Hrhow(i,j,k+2)-Hrhow(i,j,k-2)) )*ijacob3_v(i,j,k) + Krhow(i,j,k)
             Krhoe(i,j,k) = ( a5(1)*( Hrhoe(i,j,k+1)-Hrhoe(i,j,k-1)) &
                            + a5(2)*( Hrhoe(i,j,k+2)-Hrhoe(i,j,k-2)) )*ijacob3_v(i,j,k) + Krhoe(i,j,k)
          enddo
       enddo
    enddo
...
end submodule smod_flux_visc_5pts_c3
