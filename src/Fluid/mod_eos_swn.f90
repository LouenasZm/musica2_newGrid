...
!=================================================================================
module mod_ineos_swn
!=================================================================================
  !> Module to share constants of SPAN-WAGNER-non polar EoS [for inlining purposes]
!=================================================================================
  use precision
  implicit none
  ! ------------------------------------------------------------------------------
  ...
  ! ------------------------------------------------------------------------------
end module mod_ineos_swn

!=================================================================================
module mod_eos_swn
!=================================================================================
  !> Module to define subroutines for non polar SPAN-WAGNER Equation of State (EoS)
!=================================================================================
  use mod_fluid
  use mod_ineos_swn
  use warnstop
...
contains
  ...
  !===============================================================================
    ...
    psir= n01*del   *tau**(0.250_wp)              &
        + n02*del   *tau**(1.125_wp)              &
        + n03*del   *tau**(1.500_wp)              &
        + n04*del**2*tau**(1.375_wp)              &
        + n05*del**3*tau**(0.250_wp)              &
        + n06*del**7*tau**(0.875_wp)              &
        + n07*del**2*tau**(0.625_wp)*exp(-del)    &
        + n08*del**5*tau**(1.750_wp)*exp(-del)    &
        + n09*del   *tau**(3.625_wp)*exp(-del**2) &
        + n10*del**4*tau**(3.625_wp)*exp(-del**2) &
        + n11*del**3*tau**(14.5_wp )*exp(-del**3) &
        + n12*del**4*tau**(12.0_wp )*exp(-del**3)
  ...
  !===============================================================================
    ...
    d2psir_del2=                                                                                  &
        + n04*2.0_wp        *tau**(1.375_wp)                                                      &
        + n05*6.0_wp *del   *tau**(0.25_wp)                                                       &
        + n06*42.0_wp*del**5*tau**(0.875)                                                         &
        + n07               *tau**(0.625_wp)*exp(-del)   *( 2.0_wp -4.0_wp *del+del**2)           &
        + n08        *del**3*tau**(1.75_wp) *exp(-del)   *( 20.0_wp-10.0_wp*del+del**2)           &
        + n09        *del   *tau**(3.625_wp)*exp(-del**2)*(-6.0_wp +4.0_wp *del**2)               &
        + n10        *del**2*tau**(3.625_wp)*exp(-del**2)*( 12.0_wp-18.0_wp*del**2+4.0_wp*del**4) &
        + n11        *del   *tau**(14.5_wp) *exp(-del**3)*( 6.0_wp -24.0_wp*del**3+9.0_wp*del**6) &
        + n12        *del**2*tau**(12.0_wp) *exp(-del**3)*( 12.0_wp-30.0_wp*del**3+9.0_wp*del**6)
  ...
  !===============================================================================
    ...
    d3psir_del3=                                                                                          &
       + n05       *tau**(0.25_wp) *6.0_wp                                                                &
       + n06*del**4*tau**(0.875_wp)*210.0_wp                                                              &
       + n07       *tau**(0.625_wp)*exp(-del)   *(-6.0_wp +6.0_wp  *del-del**2)                           &
       + n08*del**2*tau**(1.75_wp) *exp(-del)   *( 60.0_wp-60.0_wp *del+15.0_wp*del**2-del**3)            &
       + n09       *tau**(3.625_wp)*exp(-del**2)*(-6.0_wp +24.0_wp *del**2-8.0_wp*del**4)                 &
       + n10*del   *tau**(3.625_wp)*exp(-del**2)*( 24.0_wp-96.0_wp *del**2+60.0_wp *del**4-8.0_wp *del**6)&
       + n11       *tau**(14.5_wp) *exp(-del**3)*(  6.0_wp-114.0_wp*del**3+135.0_wp*del**6-27.0_wp*del**9)&
       + n12*del   *tau**(12.0_wp) *exp(-del**3)*( 24.0_wp-186.0_wp*del**3+162.0_wp*del**6-27.0_wp*del**9)
  ...
  !===============================================================================
    ...
    d4psir_del4= n06*del**3*tau**(0.875_wp)*840.0_wp                                                                          &
         + n07       *tau**(0.625_wp)*exp(-del   )*(12.0_wp  -8.0_wp   *del+del**2)                                           &
         + n08*del   *tau**(1.75_wp )*exp(-del   )*(120.0_wp -240.0_wp *del+120.0_wp*del**2-20.0_wp*del**3+del**4)            &
         + n09*del   *tau**(3.625_wp)*exp(-del**2)*(60.0_wp  -80.0_wp  *del**2+16.0_wp*del**4)                                &
         + n10       *tau**(3.625_wp)*exp(-del**2)*(24.0_wp  -336.0_wp *del**2+492.0_wp*del**4-176.0_wp*del**6+16.0_wp*del**8)&
         + n11*del**2*tau**(14.5_wp )*exp(-del**3)*(-360.0_wp+1152.0_wp*del**3-648.0_wp*del**6+81.0_wp*del**9)                &
         + n12*tau**(12.0_wp )*exp(-del**3)*(24.0_wp-816.0_wp *del**3+1692.0_wp*del**6-756.0_wp*del**9+81.0_wp*del**12)
  ...
  !===============================================================================
    ...
    d2psir_tau2=                                                    &
       - n01*0.75_wp *0.25_wp *del   *tau**(-1.75_wp)               &
  ...
  !===============================================================================
    ...
    d3psir_tau3=                                                             &
       + n01*0.75_wp *0.25_wp *1.75_wp *del   *tau**(-2.75_wp)               &
       - n02*0.875_wp*0.125_wp*1.125_wp*del   *tau**(-1.875_wp)              &
       - n03*0.5_wp  *0.5_wp  *1.5_wp  *del   *tau**(-1.5_wp)                &
       - n04*0.625_wp*0.375_wp*1.375_wp*del**2*tau**(-1.625_wp)              &
       + n05*1.75_wp *0.75_wp *0.25_wp *del**3*tau**(-2.75_wp)               &
       + n06*1.125_wp*0.125_wp*0.875_wp*del**7*tau**(-2.125_wp)              &
       + n07*1.375_wp*0.375_wp*0.625_wp*del**2*tau**(-2.375_wp)*exp(-del   ) &
       - n08*0.25_wp *0.75_wp *1.75_wp *del**5*tau**(-1.25_wp) *exp(-del   ) &
       + n09*1.625_wp*2.625_wp*3.625_wp*del   *tau**( 0.625_wp)*exp(-del**2) &
       + n10*1.625_wp*2.625_wp*3.625_wp*del**4*tau**( 0.625_wp)*exp(-del**2) &
       + n11*12.5_wp *13.5_wp *14.5_wp *del**3*tau**( 11.5_wp) *exp(-del**3) &
       + n12*10.0_wp *11.0_wp *12.0_wp *del**4*tau**( 9.0_wp)  *exp(-del**3)
  ...
  !===============================================================================
    ...
    d2psir_tau1del1=                                                                    &
      ...
      + n02*1.125_wp              *tau**( 0.125_wp)                                     &
      + n03*1.5_wp                *tau**( 0.5_wp)                                       &
      + n04*2.0_wp*1.375_wp*del   *tau**( 0.375_wp)                                     &
      + n05*0.75_wp        *del**2*tau**(-0.75_wp)                                      &
      + n06*7.0_wp*0.875_wp*del**6*tau**(-0.125_wp)                                     &
      + n07*0.625_wp       *del   *tau**(-0.375_wp)*exp(-del)   *(2.0_wp-       del   ) &
      + n08*1.75_wp        *del**4*tau**( 0.75_wp) *exp(-del)   *(5.0_wp-       del   ) &
      + n09*3.625_wp              *tau**( 2.625_wp)*exp(-del**2)*(1.0_wp-2.0_wp*del**2) &
      + n10*3.625_wp       *del**3*tau**( 2.625_wp)*exp(-del**2)*(4.0_wp-2.0_wp*del**2) &
      + n11*14.5_wp        *del**2*tau**( 13.5_wp) *exp(-del**3)*(3.0_wp-3.0_wp*del**3) &
      + n12*12.0_wp        *del**3*tau**( 11.0_wp) *exp(-del**3)*(4.0_wp-3.0_wp*del**3)
  ...
  !===============================================================================
    ...
    d3psir_tau2del1=                                                                             &
      - n01*0.75_wp *0.25_wp               *tau**(-1.75_wp)                                      &
      + n02*0.125_wp*1.125_wp              *tau**(-0.875_wp)                                     &
      + n03*0.5_wp  *1.5_wp                *tau**(-0.5_wp)                                       &
      + n04*0.375_wp*2.0_wp*1.375_wp*del   *tau**(-0.625_wp)                                     &
      - n05*0.75_wp *0.75_wp        *del**2*tau**(-1.75_wp)                                      &
      - n06*0.125_wp*7.0_wp*0.875_wp*del**6*tau**(-1.125_wp)                                     &
      - n07*0.375_wp*0.625_wp       *del   *tau**(-1.375_wp)*exp(-del)   *(2.0_wp-       del)    &
      + n08*0.75_wp *1.75_wp        *del**4*tau**(-0.25_wp) *exp(-del)   *(5.0_wp-       del)    &
      + n09*2.625_wp*3.625_wp              *tau**( 1.625_wp)*exp(-del**2)*(1.0_wp-2.0_wp*del**2) &
      + n10*2.625_wp*3.625_wp       *del**3*tau**( 1.625_wp)*exp(-del**2)*(4.0_wp-2.0_wp*del**2) &
      + n11*13.5_wp *14.5_wp        *del**2*tau**( 12.5_wp) *exp(-del**3)*(3.0_wp-3.0_wp*del**3) &
      + n12*11.0_wp  *12.0_wp       *del**3*tau**( 10.0_wp) *exp(-del**3)*(4.0_wp-3.0_wp*del**3)
  ...
  !===============================================================================
    ...
    d3psir_tau1del2=                                                                                      &
      + n04*1.375_wp*2.0_wp        *tau**( 0.375_wp)                                                      &
      + n05*0.25_wp *6.0_wp *del   *tau**(-0.75_wp)                                                       &
      + n06*0.875_wp*42.0_wp*del**5*tau**(-0.125_wp)                                                      &
      + n07*0.625_wp               *tau**(-0.375_wp)*exp(-del)   *( 2.0_wp -4.0_wp *del+del**2)           &
      + n08*1.75_wp         *del**3*tau**( 0.75_wp) *exp(-del)   *( 20.0_wp-10.0_wp*del+del**2)           &
      + n09*3.625_wp        *del   *tau**( 2.625_wp)*exp(-del**2)*(-6.0_wp +4.0_wp *del**2)               &
      + n10*3.625_wp        *del**2*tau**( 2.625_wp)*exp(-del**2)*( 12.0_wp-18.0_wp*del**2+4.0_wp*del**4) &
      + n11*14.5_wp         *del   *tau**( 13.5_wp) *exp(-del**3)*( 6.0_wp -24.0_wp*del**3+9.0_wp*del**6) &
      + n12*12.0_wp         *del**2*tau**( 11.0_wp) *exp(-del**3)*( 12.0_wp-30.0_wp*del**3+9.0_wp*del**6)
  ...
  !===============================================================================
    ...
    s_crit= scalc_tro_swn(Tc,roc)
    
  end subroutine init_eos_swn

  !===============================================================================
  function avcalc_tro_swn(T,ro)
  !===============================================================================
  ...
  !===============================================================================
    ...
    real(wp) :: avcalc_tro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    !real(wp) :: dpdv,dpdT,v,vbm,ekTr
    ! ----------------------------------------------------------------------------

    call mpistop('procedure avcalc_tro_swn not written yet !!',0)

    avcalc_tro_swn= 1.0_wp

  end function avcalc_tro_swn

  !===============================================================================
  function c2calc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    c2calc_tro_swn= 1.0_wp/ro**2*(T/cv*dpdT**2 - dpdv)
 
    c2calc_tro_swn= c2calc_tro_swn*pcvc

  end function c2calc_tro_swn

  !===============================================================================
  function cpcalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    cpcalc_tro_swn= cv - T*dpdT**2/dpdv

    cpcalc_tro_swn= cpcalc_tro_swn*zc*rg
    
  end function cpcalc_tro_swn

  !===============================================================================
  function cvcalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    cvcalc_tro_swn= -tau**2*zci*(d2psi0_tau2(tau)+d2psir_tau2(tau,del))

    cvcalc_tro_swn= cvcalc_tro_swn*zc*rg
    
  end function cvcalc_tro_swn

  !===============================================================================
  function dpdicalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    ! ----------------------------------------------------------------------------
    real(wp), intent(in) :: Ti,roi
    real(wp) :: dpdicalc_tro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    ...
    dpdicalc_tro_swn= dpdT/cv   ! dimensionless

  end function dpdicalc_tro_swn

  !===============================================================================
  function dpdTcalc_tro_swn(Ti,roi)
  !===============================================================================
    !> Compute pressure derivative w.r.t temperature
    !> - SWN EOS -
  !===============================================================================
    ...
    ! ----------------------------------------------------------------------------
    real(wp), intent(in) :: Ti,roi
    real(wp) :: dpdTcalc_tro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    real(wp) :: T,ro,tau,del
  ...
  end function dpdTcalc_tro_swn
  
!!$  !===============================================================================
!!$  function dpdvcalc_tro_swn(Ti,roi)
!!$  !===============================================================================
...
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp), intent(in) :: Ti,roi
!!$    real(wp) :: dpdvcalc_tro_swn ! OUTPUT
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp) :: T,ro,tau,del,cv,dpdv,dpdT
!!$    ! ----------------------------------------------------------------------------
!!$
...
!!$    del= ro
!!$
!!$    dpdvcalc_tro_swn=-del**2/tau*zci*( 1.0_wp+2.0_wp*del*d1psir_del1(tau,del) &
!!$                                                + del**2*d2psir_del2(tau,del) )
!!$    
!!$    ! Dimensionalization
!!$    dpdvcalc_tro_swn= dpdvcalc_tro_swn/(roc*pc)
!!$
!!$  end function dpdvcalc_tro_swn
!!$
!!$  !===============================================================================
!!$  function d2pdv2calc_tro_swn(Ti,roi)
!!$  !===============================================================================
...
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp), intent(in) :: Ti,roi
!!$    real(wp) :: d2pdv2calc_tro_swn ! OUTPUT
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp) :: T,ro,tau,del,cv,dpdv,dpdT
!!$    ! ----------------------------------------------------------------------------
!!$
...
!!$    del = ro
!!$
!!$    d2pdv2calc_tro_swn=-del**2/tau*zci*( 1.0_wp+2.0_wp*del*d1psir_del1(tau,del) &
!!$                                                  + del**2*d2psir_del2(tau,del) )
!!$    
!!$    ! Dimensionalization
!!$    d2pdv2calc_tro_swn= d2pdv2calc_tro_swn/(roc*pc)
!!$
!!$  end function d2pdv2calc_tro_swn
!!$
!!$  !===============================================================================
!!$  function d3pdv3calc_tro_swn(Ti,roi)
!!$  !===============================================================================
...
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp), intent(in) :: Ti,roi
!!$    real(wp) :: d3pdv3calc_tro_swn ! OUTPUT
!!$    ! ----------------------------------------------------------------------------
!!$    real(wp) :: T,ro,tau,del,cv,dpdv,dpdT
!!$    ! ----------------------------------------------------------------------------
!!$
...
!!$    del = ro
!!$
!!$    d3pdv3calc_tro_swn=-del**2/tau*zci*( 1.0_wp+2.0_wp*del*d1psir_del1(tau,del) &
!!$                                                  + del**2*d2psir_del2(tau,del) )
!!$    
!!$    ! Dimensionalization
!!$    d3pdv3calc_tro_swn= d3pdv3calc_tro_swn/(roc*pc)
!!$
!!$  end function d3pdv3calc_tro_swn

  !===============================================================================
  function ecalc_pro_swn(p,ro,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    real(wp) :: ecalc_pro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    integer :: i
    !real(wp) :: T,T1,v,err
    ! ----------------------------------------------------------------------------

    call mpistop('procedure ecalc_pro_swn not written yet !!',0)

    ecalc_pro_swn= 1.0_wp

    !print *,'in ecalc_pro: at iteration',i,'error=',err
    !print *,'p:',p,'ro:',ro
    !ecalc_pro_swn= 0.0_wp
    !call mpistop('function ecalc_pro not converged',0)

  end function ecalc_pro_swn

  !===============================================================================
  function ecalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    ecalc_tro_swn= 100.0_wp + zci*(d1psir_tau1(tau,del) + d1psi0_tau1(tau))

    ecalc_tro_swn= ecalc_tro_swn*zc*pcvc
    
  end function ecalc_tro_swn

  !===============================================================================
  function dedTcalc_tro_swn(Ti,roi)
  !===============================================================================
    !> Compute internal energy from T and rho
    !> - SWN EOS -
  !===============================================================================
    ...
    ! ----------------------------------------------------------------------------
    real(wp), intent(in) :: Ti,roi
    real(wp) :: dedTcalc_tro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    real(wp) :: T,ro,tau,del
    ! ----------------------------------------------------------------------------
    ...
    dedTcalc_tro_swn= -zci*(d2psir_tau2(tau,del)+d2psi0_tau2(tau))/T**2  ! dimensionless

    !! check dimensionality
    !!dedTcalc_tro_swn= dedTcalc_tro_swn*zc*pcvc/Tc
    !!dedTcalc_tro_swn= dedTcalc_tro_swn*zc*zc

  end function dedTcalc_tro_swn

  !===============================================================================
  function gcalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    gcalc_tro_swn= 1.0_wp/(2.0_wp*ro**3*c2calc)*( d2pdv2-3.0_wp*T/cv*dpdT*d2pdvdT+ &
                         (T/cv*dpdT)**2*(3.0_wp*d2pdT2+dpdT/T*(1.0_wp-T/cv*dcvdT)) )

  end function gcalc_tro_swn

  !===============================================================================
  function pcalc_roero_swn(roei,roi,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    real(wp) :: pcalc_roero_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    integer :: i
    ...
    do i=1,20
       ...
       err = abs(T1-T)/T
       if (err.le.tol) then
          pcalc_roero_swn= del*zci/tau*(1.0_wp+del*d1psir_del1(tau,del))
          pcalc_roero_swn= pcalc_roero_swn*pc
          return
       endif
       T= T1
    enddo
    ...
    pcalc_roero_swn= 0.0_wp
    call mpistop('function pcalc_roero not converged',0)

  end function pcalc_roero_swn

  !===============================================================================
  function pcalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    pcalc_tro_swn= del*zci/tau*(1.0_wp+del*d1psir_del1(tau,del))
    pcalc_tro_swn= pcalc_tro_swn*pc
    
  end function pcalc_tro_swn

  !===============================================================================
  function rocalc_ep_swn(e,p,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    do i=1,50
       T = T1
       ro= rocalc_pt_swn(p,T,ro)
       T1= tcalc_roero_swn(ro*e,ro,T)
       
       err= abs(T1-T)/T
       if (err.lt.1.e-13_wp) then
          rocalc_ep_swn= ro
          return
       endif
    enddo
  ...
  end function rocalc_ep_swn

  !===============================================================================
  function rocalc_ps_swn(p,s,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    print *,'p:',p,'s:',s
    rocalc_ps_swn= 0.0_wp
    call mpistop('function rocalc_ps not converged',0)
    
  end function rocalc_ps_swn

  !===============================================================================
  function rocalc_pt_swn(pi,Ti,rotent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    print *,'p:',p/pc,'T:',T/tc
    rocalc_pt_swn= 0.0_wp
    call mpistop('function rocalc_pt not converged',0)

  end function rocalc_pt_swn

  !===============================================================================
  function rocalc_st_swn(si,Ti,rotent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    print *,'s:',s,'T:',T
    rocalc_st_swn= 0.0_wp
    call mpistop('function rocalc_st not converged',0)
   
  end function rocalc_st_swn

  !===============================================================================
  function scalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    scalc_tro_swn= zci*(tau*(d1psir_tau1(tau,del)+d1psi0_tau1(tau)) &
                 - psir(tau,del)-psi0(tau,del)) - s_crit

    scalc_tro_swn= scalc_tro_swn*zc*rg

  end function scalc_tro_swn

  !===============================================================================
  function tcalc_ph_swn(p,h,rotent,Ttent)
  !===============================================================================
  ...
  end function tcalc_ph_swn

  !===============================================================================
  function tcalc_pro_swn(pi,roi,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    print *,'p:',p,'ro:',ro
    tcalc_pro_swn= 0.0_wp
    call mpistop('function tcalc_pro not converged',0)

  end function tcalc_pro_swn

  !===============================================================================
  function tcalc_roero_swn(roei,roi,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    ! ----------------------------------------------------------------------------
    real(wp), intent(in) :: roei,roi,Ttent
    real(wp) :: tcalc_roero_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    integer :: i
    real(wp) :: e,ro,tau,del,err,T,T1,dedT,func,ecalc
    ! ----------------------------------------------------------------------------
    ...
    tcalc_roero_swn= 0.0_wp
    call mpistop('function tcalc_roero not converged',0)
    
  end function tcalc_roero_swn

  !===============================================================================
  function tcalc_sro_swn(si,roi,Ttent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    do i=1,20
       ...
       if (err.le.tol) then
          tcalc_sro_swn= 1.0_wp/tau
          tcalc_sro_swn= tcalc_sro_swn*Tc
          return
       endif
       tau= tau1
    enddo
    ...
    print *,'s :',s,'ro:',ro
    tcalc_sro_swn= 0.0_wp
    call mpistop('function tcalc_sro not converged',0)
    
  end function tcalc_sro_swn

  !===============================================================================
  subroutine tcalc_Hstot_swn(T,ro,p,H_tot,s_tot,am0,coeff)
  !===============================================================================
  ...
  !===============================================================================
   ...
   ! input: interior part of Riemann invariant am0
    ...
    do i=1,1000
       ...
       ro=rocalc_st_swn(s_tot,T1,ro1)
              
       ! update pressure
       p=pcalc_tro_swn(T1,ro)
    
       ! update pressure derivative wrt T
       dpdT=dpdicalc_tro_swn(T1,ro)*cvcalc_tro_swn(T1,ro)

       ! update temperature
       
       fn=ecalc_tro_swn(T1,ro)+p/ro+0.5_wp*coeff*(am0-p)**2-H_tot

       der_fn=dedTcalc_tro_swn(T1,ro)+dpdT/ro-coeff*(am0-p)*dpdT
       ...
       if (err.le.tol*10.) then
          ro=rocalc_st_swn(s_tot,T,ro1)
          p=pcalc_tro_swn(T,ro)
          return
       endif
  ...
  end subroutine tcalc_Hstot_swn
  
  !===============================================================================
  function vvol_swn(pi,Ti,vtent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    real(wp) :: vvol_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    integer  :: i
    real(wp) :: p,T,ro,del,tau,v0,v1,dpdv,err
    ! ----------------------------------------------------------------------------
    ...
    do i=1,15000
       ...
       v1= v0 - (p-pcalc_tro_swn(Ti*Tc,roc/v0)/pc)/(-dpdv)
  ...
  end function vvol_swn

  !===============================================================================
  function vvol_d1_swn(Ti,vtent)
  !===============================================================================
  ...
  !===============================================================================
    ...
    real(wp) :: vvol_d1_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    integer  :: i
    real(wp) :: T,ro,del,tau,err
  ...
  end function vvol_d1_swn

  !===============================================================================
  function vvol_d2_swn(Ti,vtent)
  !===============================================================================
  ...
  end function vvol_d2_swn

  !===============================================================================
  function intpcalc_tro_swn(Ti,roi)
  !===============================================================================
  ...
  !===============================================================================
    ...
    ! ----------------------------------------------------------------------------
    real(wp), intent(in) :: Ti,roi
    real(wp) :: intpcalc_tro_swn ! OUTPUT
    ! ----------------------------------------------------------------------------
    real(wp) :: T,ro,tau,del
    ! ----------------------------------------------------------------------------
    ...
    intpcalc_tro_swn=-1.0_wp/(zc*tau)*(log(del)+psir(tau,del))
    
  end function intpcalc_tro_swn

end module mod_eos_swn
