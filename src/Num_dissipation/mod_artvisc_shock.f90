...
contains
  ...
  !============================================================================
  subroutine init_artvisc_shock(dissip_coeff,dissip_shock)
  !============================================================================
...
!!$    xnu0=dissip_shock
  ...
  end subroutine init_artvisc_shock

  !==============================================================================
  subroutine artvisc_o9_shock
  !==============================================================================
  ...
  !==============================================================================
    ...
    ! ---------------------------------------------------------------------------
    integer :: i,j,k
    real(wp) :: uc1,uc2,uc,vc1,vc2,vc
    real(wp) :: wc1,wc2,wc,srm12,srp12,sr
...
!                                       abs(prs(i-1,j,k)+2.0_wp*prs(i,j,k)+prs(i+1,j,k))
    ...
    if (is_curv3) then
       do k=1,nz
          do j=1,ny
             do i=ndx_d,nfx_d
                ! contravariant velocity at i+1/2 (uc2) and i-1/2 (uc1)
                uc=(uu(i,j,k)*ksi_x(i,j,k)+vv(i,j,k)*ksi_y(i,j,k)+ww(i,j,k)*ksi_z(i,j,k))*ijacob3(i,j,k)
                uc2=(uu(i+1,j,k)*ksi_x(i+1,j,k)+vv(i+1,j,k)*ksi_y(i+1,j,k)+ww(i+1,j,k)*ksi_z(i+1,j,k))*ijacob3(i+1,j,k)
                uc1=(uu(i-1,j,k)*ksi_x(i-1,j,k)+vv(i-1,j,k)*ksi_y(i-1,j,k)+ww(i-1,j,k)*ksi_z(i-1,j,k))*ijacob3(i-1,j,k)
                ! compute spectral radius at i+1/2 (srp12) and i-1/2 (srm12)
                sr=abs(uc)+c_(i,j,k)*g3_ksi(i,j,k)
                srp12=0.5_wp*(abs(uc2)+c_(i+1,j,k)*g3_ksi(i+1,j,k)+sr)
                srm12=0.5_wp*(abs(uc1)+c_(i-1,j,k)*g3_ksi(i-1,j,k)+sr)
                ! compute dissipation term: delta_i/delta_x=(D_{i+1/2}-D_{i-1/2})/dksi
                Krho(i,j,k) = srm12*Drho(i-1,j,k) -srp12*Drho(i,j,k)
                Krhou(i,j,k)= srm12*Drhou(i-1,j,k)-srp12*Drhou(i,j,k)
                Krhov(i,j,k)= srm12*Drhov(i-1,j,k)-srp12*Drhov(i,j,k)
                Krhow(i,j,k)= srm12*Drhow(i-1,j,k)-srp12*Drhow(i,j,k)
                Krhoe(i,j,k)= srm12*Drhoe(i-1,j,k)-srp12*Drhoe(i,j,k)
             enddo
          enddo
       enddo
    elseif (is_curv) then
       ...
       do k=1,nz
          do j=1,ny
             do i=ndx_d,nfx_d
                ...
                Krho(i,j,k) = (srm12*Drho(i-1,j,k) -srp12*Drho(i,j,k))*idx(i)
                Krhou(i,j,k)= (srm12*Drhou(i-1,j,k)-srp12*Drhou(i,j,k))*idx(i)
                Krhov(i,j,k)= (srm12*Drhov(i-1,j,k)-srp12*Drhov(i,j,k))*idx(i)
                Krhow(i,j,k)= (srm12*Drhow(i-1,j,k)-srp12*Drhow(i,j,k))*idx(i)
                Krhoe(i,j,k)= (srm12*Drhoe(i-1,j,k)-srp12*Drhoe(i,j,k))*idx(i)
                ! store dissipation coefficient
                !uvar(i,j,k,1)=sr
             enddo
          enddo
       ...
       do k=1,nz
          do j=ndy_d,nfy_d
             do i=1,nx
                ...
                vc2=(uu(i,j+1,k)*eta_x(i,j+1,k)+vv(i,j+1,k)*eta_y(i,j+1,k)+ww(i,j+1,k)*eta_z(i,j+1,k))*ijacob3(i,j+1,k)
                vc1=(uu(i,j-1,k)*eta_x(i,j-1,k)+vv(i,j-1,k)*eta_y(i,j-1,k)+ww(i,j-1,k)*eta_z(i,j-1,k))*ijacob3(i,j-1,k)
                ...
                Krho(i,j,k) = Krho(i,j,k) +srm12*Drho(i,j-1,k) -srp12*Drho(i,j,k)
                Krhou(i,j,k)= Krhou(i,j,k)+srm12*Drhou(i,j-1,k)-srp12*Drhou(i,j,k)
                Krhov(i,j,k)= Krhov(i,j,k)+srm12*Drhov(i,j-1,k)-srp12*Drhov(i,j,k)
                Krhow(i,j,k)= Krhow(i,j,k)+srm12*Drhow(i,j-1,k)-srp12*Drhow(i,j,k)
                Krhoe(i,j,k)= Krhoe(i,j,k)+srm12*Drhoe(i,j-1,k)-srp12*Drhoe(i,j,k)
             enddo
          enddo
       enddo
    elseif (is_curv) then
       ...
       do k=1,nz
          do j=ndy_d,nfy_d
             do i=1,nx
                ...
                ! compute dissipation term: delta_j/delta_y=(D_{j+1/2}-D_{j-1/2})/dy
                Krho(i,j,k) = Krho(i,j,k) +(srm12*Drho(i,j-1,k) -srp12*Drho(i,j,k) )*idy(j)
                Krhou(i,j,k)= Krhou(i,j,k)+(srm12*Drhou(i,j-1,k)-srp12*Drhou(i,j,k))*idy(j)
                Krhov(i,j,k)= Krhov(i,j,k)+(srm12*Drhov(i,j-1,k)-srp12*Drhov(i,j,k))*idy(j)
                Krhow(i,j,k)= Krhow(i,j,k)+(srm12*Drhow(i,j-1,k)-srp12*Drhow(i,j,k))*idy(j)
                Krhoe(i,j,k)= Krhoe(i,j,k)+(srm12*Drhoe(i,j-1,k)-srp12*Drhoe(i,j,k))*idy(j)
                ! store dissipation coefficient
                !uvar(i,j,k,2)=sr
             enddo
          enddo
...
!                                          abs(prs(i,j,k-1)+2.0_wp*prs(i,j,k)+prs(i,j,k+1))
       ...
       if (is_curv3) then
          do k=1,nz
             do j=ndy_d,nfy_d
                do i=1,nx
                   ! contravariant velocity at j+1/2 (wc2) and j-1/2 (wc1)
                   wc=(uu(i,j,k)*phi_x(i,j,k)+vv(i,j,k)*phi_y(i,j,k)+ww(i,j,k)*phi_z(i,j,k))*ijacob3(i,j,k)
                   wc2=(uu(i,j,k+1)*phi_x(i,j,k+1)+vv(i,j,k+1)*phi_y(i,j,k+1)+ww(i,j,k+1)*phi_z(i,j,k+1))*ijacob3(i,j,k+1)
                   wc1=(uu(i,j,k-1)*phi_x(i,j,k-1)+vv(i,j,k-1)*phi_y(i,j,k-1)+ww(i,j,k-1)*phi_z(i,j,k-1))*ijacob3(i,j,k-1)
                   ! compute spectral radius at j+1/2 (srp12) and j-1/2 (srm12)
                   sr=abs(wc)+c_(i,j,k)*g3_eta(i,j,k)
                   srp12=0.5_wp*(abs(wc2)+c_(i,j,k+1)*g3_phi(i,j,k+1)+sr)
                   srm12=0.5_wp*(abs(wc1)+c_(i,j,k-1)*g3_phi(i,j,k-1)+sr)
                   ! compute dissipation term: delta_j/delta_y=(D_{j+1/2}-D_{j-1/2})/dy
                   Krho(i,j,k) = Krho(i,j,k) +srm12*Drho(i,j-1,k) -srp12*Drho(i,j,k)
                   Krhou(i,j,k)= Krhou(i,j,k)+srm12*Drhou(i,j-1,k)-srp12*Drhou(i,j,k)
                   Krhov(i,j,k)= Krhov(i,j,k)+srm12*Drhov(i,j-1,k)-srp12*Drhov(i,j,k)
          ...
          do k=ndz_d,nfz_d
             do j=1,ny
                do i=1,nx
                   ...
                   Krho(i,j,k) = Krho(i,j,k) +(srm12*Drho(i,j,k-1) -srp12*Drho(i,j,k) )*idz(k)
                   Krhou(i,j,k)= Krhou(i,j,k)+(srm12*Drhou(i,j,k-1)-srp12*Drhou(i,j,k))*idz(k)
                   Krhov(i,j,k)= Krhov(i,j,k)+(srm12*Drhov(i,j,k-1)-srp12*Drhov(i,j,k))*idz(k)
                   Krhow(i,j,k)= Krhow(i,j,k)+(srm12*Drhow(i,j,k-1)-srp12*Drhow(i,j,k))*idz(k)
                   Krhoe(i,j,k)= Krhoe(i,j,k)+(srm12*Drhoe(i,j,k-1)-srp12*Drhoe(i,j,k))*idz(k)
                enddo
             enddo
          enddo
       endif
  ...
  end subroutine artvisc_o9_shock

  !==============================================================================
  subroutine artvisc_o7_shock
  !==============================================================================
...
!                                          abs(prs(i,j,k-1)+2.0_wp*prs(i,j,k)+prs(i,j,k+1))
  ...
  end subroutine artvisc_o7_shock

  !==============================================================================
  subroutine artvisc_o5_shock
  !==============================================================================
...
!                                          abs(prs(i,j,k-1)+2.0_wp*prs(i,j,k)+prs(i,j,k+1))
  ...
  end subroutine artvisc_o5_shock

  !==============================================================================
  subroutine artvisc_o3_shock
  !==============================================================================
...
!                                          abs(prs(i,j,k-1)+2.0_wp*prs(i,j,k)+prs(i,j,k+1))
  ...
  end subroutine artvisc_o3_shock

end module mod_artvisc_shock
